...
public class Settings {
    public static int getGridSize(Context context) {
        ...
        if(gridSize == 0) gridSize = Integer.valueOf(prefs.getString("customGameSizePref", Integer.toString(context.getResources().getInteger(R.integer.DEFAULT_BOARD_SIZE))));
    ...
    public static String getPlayer1Name(Context context, GoogleApiClient gamesClient) {
        if(gamesClient.isConnected()) return Games.Players.getCurrentPlayer(gamesClient).getDisplayName().split(" ")[0];
