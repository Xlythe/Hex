...
public class XMLHandler extends DefaultHandler {
    ...
    public XMLHandler(GameObject game) {
        this.game = game;
    }
     ...
     * xml is like: <tag attribute="attributeValue">
     */
    @Override
        ...
        // For list of available games
            ...
            if(in_session) {
                if(localName.equals("member")) {
                    parsedDataset.addSessionMember(Integer.parseInt(atts.getValue("plc")), Integer.parseInt(atts.getValue("uid")), atts.getValue("nam"),
                            atts.getValue("stat"), 0, 0);
                }
            }
        }
        // For playing a game
            ...
            if(localName.equals("playerList")) {
                ...
                if(localName.equals("player")) {
                    parsedDataset.addPlayer(Integer.parseInt(atts.getValue("place")), Integer.parseInt(atts.getValue("uid")), atts.getValue("name"),
                            atts.getValue("stat"), Integer.parseInt(atts.getValue("timerLeft")), currentTime - Long.parseLong(atts.getValue("lastRefresh")));
                }
            }
            ...
            if(localName.equals("eventList")) {
                ...
                if(localName.equals("event")) {
                    ...
                    // Moves
                    else if(atts.getValue("type").equals("MOVE") && game != null) {
                        ...
                        for(int i = 0; i < parsedDataset.players.size(); i++) {
                            if(parsedDataset.players.get(i).uid == Integer.parseInt(atts.getValue("uid"))) {
                                if(parsedDataset.players.get(i).place == 1) {
                                    parsedDataset.p1moves.add(GameAction.stringToPoint(point, game));
                                }
                                else if(parsedDataset.players.get(i).place == 2) {
                                    parsedDataset.p2moves.add(GameAction.stringToPoint(point, game));
                                }
                                break;
                            }
                        }
